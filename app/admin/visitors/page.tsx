...
export default async function AdminVisitorsPage() {
  ...
  // Fetch unique user profiles for activities that have user_id
  const userIds = [...new Set(activities?.filter((a) => a.user_id).map((a) => a.user_id) || [])]
  const { data: profiles } =
    userIds.length > 0
      ? await supabase.from("profiles").select("id, email, full_name, account_number").in("id", userIds)
      : { data: [] }
                      ...
                      <td className="px-6 py-4">
                        {profile ? (
                          <>
                            ...
                            <div className="text-xs text-gray-400 font-mono">{profile.account_number}</div>
                          </>
                        ) : (
